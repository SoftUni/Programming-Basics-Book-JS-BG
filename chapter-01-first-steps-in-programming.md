...
Нека разгледаме по-детайлно **тялото на функцията**. На 10-ти ред **декларираме променливата** **`firstNumber`**, като ѝ **присвояваме** стойността на текстовото поле с **`id=firstNumber`**. Функцията **`parseInt(...)`** ни подсигурява, че въведеният текст ще бъде конвертиран до число. След това, по аналогичен начин получаваме стойността на второто текстово поле (с **`id=secondNumber`**). Накрая използвайки същия механизъм, вместо да **вземаме стойността** на третото поле с с **`id=result`**, ние му **присвояваме** стойност, като го поставяме от лявата страна на равенството.
