...
<div class="video-player">
  ...
  href="https://youtu.be/0YkrJsKwHdM">
  https://youtu.be/0YkrJsKwHdM</a>.
</div>
...
Често ще чувате че даден код се **"изпълнява на клиента"** - това ще рече, че JavaScript кодът, се изпълнява във вашият **browser**, който играе ролята на клиент, или приемник. За да има приемник, трябва да има и предавател. В технологичният свят тези предаватели се наричат сървъри. Можете да си представяте сървърите като едни много мощтни компютри, до които имат достъп много хора. Всички сайтове стоят на подобни сървъри - т.е. файловете които помагат на сайта да изглежда такъв какъвто го виждате - **картинки, текстове, JavaScript файлове**, се намират на конкретен сървър. Вашият **browser** (клиент) се свързва към **сървъра**, на койтo се намира сайта, който посещавате и сървърът **изпраща** обратно необходимите файлове, за да се визуализира пред вас любимият сайт. Можете да си представите, че вашият browser е **радиото** в колата ви на което слушате любимата програма, а сървърът е сградата от където се излъчва това предаване, макар и комуникацията между двете да протича по различен начин.

Другият популярен **интерпретатор за JavaScript** e **NodeJS**. Можете да си го представите като приложение, което инсталирате на компютъра си и той започва да разбира от **JavaScript** по същият начин, по който и вашият browser разбира. По този начин можете да изпълнявате JavaScript код **директно на компютъра си**, без необходимостта от browser. Както казахме сървърите са просто по - мощни компютри. Те започват да разбират от JavaScript по същият начин по който и вашият компютър - просто им се инсталира **NodeJS**. Можете да си инсталирате **NodeJS** от официалния сайт https://nodejs.org напълно безплатно, като следвате инструкциите.
...
Програмата се пише в текстов формат, а самият текст на програмата се нарича **сорс код** (source code). Той се запазва във файл с разширение .js (например **`main.js`**) след което може да се изпълни през вашият browser или през **конзолата** с помощта на **NodeJS**.

### Компютърни програми – примери

Да започнем с много прост пример за кратка JavaScript програма.

#### Пример: програма, която нотифицира потребителя

Нашата първа програма ще е единична JavaScript команда, която нотифицира потребителя, че е спечелил 1 000 000 долара, както често се случва, когато разглеждате някой сайт с много реклами.

```javascript
alert("Congratulations!!! You have just won $1 000 000!");
```

След малко ще разберем как можем да изпълним тази команда и да видим добре познатото съобщение, но засега нека само разгледаме какво представляват командите в програмирането. Да се запознаем с още няколко примера.

#### Пример: програма, която спами нотификации на потребителя

Можем да усложним предходната програма, като зададем за изпълнение повтарящи се в цикъл команди за нотифициране на потребителя, че е побител в томбола - нещо кеото също доста често се случва:
...
Разгледахме **три примера за компютърни програми**: единична команда, серия команди в цикъл и поредица от 4 команди. Нека сега преминем към по-интересното: как можем да пишем собствени програми на **JavaScript** и как можем да ги компилираме и изпълняваме?
...
В следващите редове подробно са описани подробно **стъпките за инсталация на Visual Studio Code** (версия 1.19.1). След като свалим инсталационния файл и го стартираме, се появява следният екран:
...
Сега е моментът да си направим средата възможно най - приятна за разработка. Visual Studio Code е едно от IDE-тата с най - голями възможности за персонализация. Най - често тази персонализация става под формата на **extension**-и. Списък с всички разширения, можете да намерите на официалният сайт [https://marketplace.visualstudio.com/](https://marketplace.visualstudio.com/). Имайте предвид, че голяма част от тези разширения са строго специфични при работа с конкретен език за програмиран.

Препоръчваме ви като за начало да си инсталирате следните 2 extension-a, които драстично ще подобрят работата ви при писане на **JavaScript** : 
...
2. В полето за търсене изписваме името на extension-a, който желаем да инсталираме
3. От получените резултати избираме този, който ни удоволетворява
4. Натискаме install бутона
5. Рестартираме Visual Studio Code.

![](assets/chapter-1-images/00.Visual-studio-04.png)

Това е всичко. Готови сме за работа с Visual Studio Code и **JavaSciprt**.
...
Алтернативно за бързо тестване можем да използваме и директно своят браузър с натискане на **[F12]**, но това генерално е опция ако искате да изпробвате просто нещо наистина на бързо.

## Пример: създаване на конзолна програма "Hello JavaScript"

Да се върнем на нашата конзолна програма. Вече имаме Visual Studio Code и можем да го стартираме. След това създаваме нов js файл: [**File**] &rarr; [**New File**] &rarr;

![](assets/chapter-1-images/00.Visual-studio-05.png)

След това е важно да запаметтим нашият файл, като задължително го записваме с разширение ".js". Също така му задаваме **смислено име** на нашата програма, например `helloJS`:

![](assets/chapter-1-images/00.Visual-studio-06.png)

### Писане на програмен код

Писането на JavaScript код не изисква никаква допълнителна подготовка от това, което вече направихме - да си създадем файл с разширение **.js**. Затова директно пристъпваме към писането на първият си ред код.

Изписваме следната команда:
...
## Изпълняване на код в браузър

До тук видяхме как можем да си направим и да изпълним конзолна програма. Нека сега видим как можем да напишем код, който да се изпълнява в нашият браузър. По подобен начин се правят и всички сайтове, които посещавате. 

Всъщност принципът е много подобен на това, кеото направихме току що. Разликата е, че когато си създаваме нов файл, не му даваме разширение ".js", a **.html**. След това единственото, което трябва да направим е да оградим кодът си със **`<sciprt>`** отварящият се и **`</script>`**, затварящ се **html таг**. По подобен начин преди малко оградихме кодът си, за да може да бъде тестван в Judge системата. Ето така би изглеждал нашият код във Visual Studio Code сега:

![](assets/chapter-1-images/01.Hello-js-05.png)

При този подход, сега остава единствено да намерите файла **helloJS.html**, на мястото, където сте го запаметтили и да кликнете два пъти върху него. Той ще се зареди във браузърът ви, но за да видите резултатът от изпълнението ще трябва да натиснете **F12**, което ще зареди конзолата на вашият браузър. Ние сме задали команда за печатане на конзолата, затова и трябва да я покажем.

![](assets/chapter-1-images/01.Hello-js-06.png)

Сега, след като вече **знаете как да изпълнявате програми**, можете да тествате примерните програми по-горе, които показват нотификации на потребителя. Позабавлявайте се, пробвайте тези програми. Пробвайте да ги промените и да си поиграете с тях. Заменете командата **`console.log("Hello JavaScript");`** с команда **`console.error("Error occured");`** и стартирайте програмата. Забележете, че програмите с нотификациите, могат да бъдат изпъление само през вашият браузър, а когато опитате да ги изпълните през конзолата програмата ви дава грешка. Това е така, защото конзолата няма функционалност да нотифицира чрез визуални елементи, какъвто е alert-a.
...
Липсата на **точка и запетая** (**`;`**) в края на командите е един от вечните проблеми на начинаещия програмист. Пропускането на този знак може да доведе до **неправилно функциониране на програмата** и **често проблемът остава незабелязан**. Ето примерен грешен код:
...
На първо място научихме **какво е програмирането** - **задаване на команди, изписани на компютърен език**, които машината разбира и може да изпълни. Разбрахме още какво е **компютърната програма** - тя представлява **поредица от команди**, подредени една след друга. Запознахме се с **езика за програмиране JavaScript** на базисно ниво и как **да създаваме прости конзолни програми и web програми** с Visual Studio Code. Видяхме как да печатаме с **`console.log(…)`** и как да стартираме програмата си с [**F5**]. Научихме се да тестваме кода си в **SoftUni Judge**.
...
Тествайте решението си тук. Не забравяйте да оградите кодът си със **solve** функцията:
...
Създаваме нов **JavaScript файл** с име “**nums1To20.js**“:
Изпосваме 20 команди **``console.log(…)``**, всяка на отделен ред, за да отпечатаме числата от 1 до 20 едно след друго. По-досетливите от вас, сигурно се питат дали няма по-умен начин. Спокойно, има, но за него по-късно.
...
Правим нов **JavaScript файл**. За момента този тип програми ще тестваме само в **Judge системата**, която има изграден механизъм за подаване на входни данни към програмата. За да **получим двете числа**, трябва да декларирам това свое желание, като променим ограждащият код, който свикнахме да пишем:
...
На първите 2 реда декларираме, че в момента ще опишем една **HTML страница**, използвайки таговете **`<html>`** и **`<body>`**, който декларира започването на тялото на нашата страница - или нейната основна част, която ще бъде визуализирана. Последните 2 реда имаме съответните **затварящи тагове**, които декларират край на областта. Може да забележете, че се различават от **отварящите тагове** по наклонената черта пред името - **`</body>`**.

В тялото на нашата страничка, чрез използването на **HTML тагове** описваме това което искаме да се визуализира а именно - 3 полета, в които може да се въвеждат числа. Правим това с тага **`<input type="number">`**. Използвайки този таг ние декларираме, че искаме да имаме визуализация за вход от типа число. Допълнителният **атрибут** - **`id`** служи за да посочим уникалното име на този **таг**. Това име е напълно по наш избор. Това id ще ни трябва в последствие, за да можем да получим информация за точно този **HTML елемент**.

След това на 7ми ред имаме деклариран друг **`input`** като този път типът му е **button**. По този начин указваме, че искаме да имаме елемент, върху който може да се натиска с бутоните на мишката и това да доведе до някакви резултати. 

Нека сега разгледаме и **JavaScript кодът**, който написахме: 

![](assets/chapter-1-images/07.Sumator-05.png)

Първо **декларираме функция** **`calculate`** която прочита информацията от първите 2 текстови полета, след което изчислява сумата и я **присвоява** като стойност на третото поле. Функцията продължава от 9ти до 17ти ред. Какво представляват **функциите** - как се **декларират** и **извикват**, ще научим малко по - късно в тази книга.

Нека разгледаме по - детайлно **тялото на функцията**. На 10ти ред **декларираме променлива** **`firstNumber`**, като ѝ **присвояваме** стойността на текстовото поле с `id` **firstNumber**. Функцията **`parseInt`** ни подсигурява че въведеният текст ще бъде число, а не някаква думичка. След това по аналогичен начин получаваме стойността на второто текстово поле. Накрая използвайки въщият механизъм, вместо да **вземаме стойността** на третото поле с `id` **result**, ние му **присвояваме** стойност, като го поставяме от лявата част на равенството.

След това на 19ти и 20ти ред достъпваме нашият бутон и му казваме да **слуша** за **`click`** събития върху него и когато се получи такова събитие, да се **изпълни** функцията **`calculate`**, която **декларирахме** преди малко. С други думи казано когато кликнем с мишката върху нашият бутон, ще се изпълни функцията **`calculate`**.

Приложението е готово. Можем да го стартираме като намерим файлът във файловата си система и го отворим. Той ще се зареди в браузърът ни по подразбиране.
...
Целта на горният пример (уеб приложение) не е да се научите, а да се докоснете по-надълбоко до програмирането, **да разпалите интереса си** към разработката на софтуер и да се вдъхновите да учите здраво. **Имате да учите още много**, но пък е интересно, нали?
