...
Повече информация за разликите между двата вида оператори за сравнение и различие можете да получите от тук: 
...
Използването на ключовата дума **`var`** за дефиниране на променливи е практика, която **в миналото** е била главният начин за деклариране, но вече е **непрепоръчително** да бъде прилагана. Затова и всички примери в тази книга ще бъдат давани  с **`let`**.

Важно е да се отбележи, че съществува и **трети начин** за инициализиране на променливи - чрез използването на ключовата дума **`const`**. Тези променливи имат същия обхват както дефинираните чрез  **`let`**, но имат една съществена разлика - приемат характеристиките на **константна променлива**. Това означава, че след първоначалното им деклариране, тяхната стойност е **невъзможно да бъде променяна или предефинирана**.
...
![](assets/chapter-3-1-images/01.ЕxcellentResult-input.png)
...
Както обикновено, първо трябва да добавим **нов JavaScript файл**. За кода на програмата ни е необходима единична **`if-else`** конструкция. Може да си помогнете частично с кода от картинката, който е умишлено замъглен, за да помислите как да го допишете сами:  
