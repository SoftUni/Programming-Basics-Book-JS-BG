...
Плъгин (приставка), написан от **Дейвид Бау** за jQuery - **jQuery-turtle** , която дава набор от функции за рисуване на графики от тип **"костенурка"**.<br>
...
Векторна графика, която иползваме с цел по-добра презентация на нашето приложение.


***
Придържайки се към документацията на **jQuery-turtle**, трябва да инициализираме нашия обект и да задададем основни характеристики на "костенурката". Със следния код ще определим размера (**turtleScale**) на графиката и скоростта (**turtleSpeed**) на движение.
...
След като сме готови с основата на нашето приложение, остава да напишем функциите за всеки бутон. За тази цел използваме предварително зададените от нас имена (**ID**) на обектите в **html файла**. Ще споделим кода за първите три бутона, за да се запознаете с основните принципи.

+ **Бутон "Draw"**

Слагаме функция към елемента с **ID** (име) - **justDraw** , която да се активира в момента на кликване върху бутона.
```js
...
Слагаме функция към елемента с **ID** (име) - **reset** , която да се активира в момента на кликване.
...
```
 Чрез **window.location.reload()** aктивираме презареждане на прозореца, с което нулираме текущото му състояние.

+ **Бутон "Hide Turtle"**

Слагаме функция към елемента с **ID** (име) - **hide** , която да се активира при кликване върху бутона.
...
```
Използваме готова функция от библиотеката **jQuery - toggle()**. Чрез нея скриваме и показваме елементи. Отделно ще прикачим друга функция, която да променя текста в бутона при кликване. Трябва да използваме ключовата дума **this**. Тя има съществена роля в синтаксиса на библиотеката и мислено може да я заменим със **self/itself**.<br/>
В конкретния случай е равна на елемента **"hide"** т.е **this = #hide**. Обръщаме се към самия елемент и задаваме функция за смяна на текста, която също се активира с всяко направено кликване върху бутона.

Остава да решим проблема с автоматичното изтриване на полето за рисуване при натискане на нов бутон.<br> Не искаме фигурите да се чертаят една върху друга или всеки път да използваме бутона **Reset**.
