...
За изчисленията на изходните параметри използваме **функцията** **`.toFixed(Number)`**. Тази функция **закръгля десетично** число до **зададен брой цифри** след десетичния знак. За целта, използваме функцията, за да закръглим двете ни променливи (**`studioRent`**, **`apartamentPrice`**). В нашия случай ще закръглим десетичното число до **две цифри** след десетичната точка.
