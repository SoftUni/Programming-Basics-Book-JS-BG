...
- Как да работим с **типове данни и променливи**, които са ни необходими при обработка на числа и низове?
...
В програмирането всяка една променлива съхранява определена **стойност** от даден **тип**. Типовете данни могат да бъдат например: **число**, **текст** (низ), **булев** тип, **дата**, **списък** и др.
Ето няколко примера за типове данни и стойности за тях:
- **number** - тип число: 1, 42, -5, 3.14, NaN, …
- **string** - тип текст (низ): 'Здрасти', "Hi", 'Beer', …
- **boolean** - булев тип: true, false
- **Date** - дата: Tue Jul 04 2017, …

[TODO: Variables scope var/let]?
...
На първият ред **`function calculateSquareArea([input]) {`** дефинираме нашата функция, като и даваме име и задаваме аргументите които ще чете. В нашия случай имаме един аргумент, който ще представлява страна на квадрата.

Следващият ред **`let a = Number(input)`** взима цяло число от аргумента на функцията. Всъщност първо се прочита текст (низ) и след това се преобразува до цяло число (парсва се) чрез **`Number(input);`**. Резултатът се записва в променлива с име **`a`**.
...
За да прочетем **текст** е необходимо да **дефинираме аргимент** на нашата функция:
