...
* **`Math.round(…)`** - закръглянето се извършва по **основното правило за закръгляване** - ако десетичната част е по-малка 5, закръглението е надолу и обратно, ако е по-голяма от 5 - нагоре:
```javascript
Math.round(5.439); // 5
Math.round(5.539); // 6
```
