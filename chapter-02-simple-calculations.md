...
При печат на текст, числа и други данни, **можем да ги съединим**, като използваме шаблони **``` `variable = ${variable}` ```**. В програмирането тези шаблони се наричат **placeholders**. 
...
Ако стартираме функцията и въведем за страните съответно `3`, `4` и `5`, ще получим следния резултат:
```javascript
