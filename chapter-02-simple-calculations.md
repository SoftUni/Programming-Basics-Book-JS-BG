...
При печат на текст, числа и други данни, **можем да ги съединим**, като използваме шаблони **``` `variable = ${variable}` ```**. В програмирането тези шаблони се наричат **placeholders**. Обърнете внимание, че за да бъде разпознат шаблонът, трябва да използваме наклонен апостроф <code><strong>`</strong></code> (**backtick**), вместо обикновени кавички:
