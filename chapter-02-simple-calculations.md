...
Следващият ред **`let a = Number(input)`** взима цяло число от аргумента на функцията. Всъщност първо се прочита текст (стринг) и след това се преобразува до цяло число (парсва се) чрез **`Number(input);`**. Резултатът се записва в променлива с име **`a`**.
