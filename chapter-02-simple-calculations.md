...
Опитайте да въведете грешно число, например "**hello**". Ще получите съобщение за грешка по време на изпълнение (exception). Това е нормално. По-късно ще разберем как можем да прихващаме такива грешки и да връщаме по-смислени за потребителя съобщения.
...
Следващият ред **`let a = Number(arg1)`** взима цяло число от аргумента на функцията. Всъщност първо се прочита текст (стринг) и след това се преобразува до цяло число (парсва се) чрез **`Number(arg1);`**. Резултатът се записва в променлива с име **`a`**.
...
За да прочетем потребителски вход под формата на **дробно число** отново е необходимо да **дефинираме аргимент** на нашата функция. Синтаксисът е същия както при четене на цяло число:
