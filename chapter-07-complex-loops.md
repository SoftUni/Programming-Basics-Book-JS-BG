...
Можем да решим задачата чрез следната поредица от действия (алгоритъм):
  ...
  * Създаваме метод, който ще приема числото **`n`**
...
* В **тялото на цикъла**: отпечатваме стойността на текущото число и го увеличаваме, използвайки формулата от условието на задачата.
...
* Създаваме променлива **`num`**, на която присвояваме целочислената стойност на пърия аргумент, подаден на метода.
* За условие на цикъла слагаме израз, който е **`true`**, ако числото **не е** в диапазона, посочен в условието.
* В **тялото на цикъла**: отпечатваме съобщение със съдържание "**Invalid number!**" на конзолата, след което присвояваме нова стойност за **`num`** (следващия аргумент, подаден на метода).
...
* Създаваме променливи **`a`** и **`b`**, на които присвояваме **целочислени** стойности, подадени на метода.
...
* Създаваме променливата **`n`**, на която присвояваме целочислена стойност подадена на метода.
...
* Създаваме променливата **`n`**, на която присвояваме цялото число, което е подадено на метода.
...
* В **тялото на цикъла**:
   * Вземаме целочислена стойност, която е подадена на метода и я присвояваме на **`n`**.
...
Конструкцията **`try-catch`** има различни варианти, но за сега ще се запознаем само с най-основния от тях: 
 ...
 ![](assets/old-images/chapter-7-images/00.Try...catch-01.png)
...
![](assets/old-images/chapter-7-images/11.Wrong-numbers-try-catch-01.png)
...
Сега добавяме **картинките** за играта (те са част от файловете със заданието за този проект и могат да бъдат свалени от [тук](https://github.com/SoftUni/Programming-Basics-Book-CSharp-BG/tree/master/assets/old-images/chapter-7-assets)). Копираме ги от Windows Explorer и ги поставяме в папката **"images"** в [**Solution Explorer**] във Visual Studio с **copy/paste**.
...
За целта добавяме действията [**New Game**] и [**Fire Top**] / [**Fire Bottom**] в контролера **`HomeController.cs`**:
