...
Входът е масив състоящ се от **две цели числа** (аргумента): **n** и **l** в интервала [**1 … 9**].
...
* **Прочитане и конвертиране на входните данни** – в настоящата задача това включва вземането на два елемента от подадения масив **`n`** и **`l`** и преобразуването им в числа.
...
Един от начинте да намерим решението на тази задаче е да създадем **пет** **`for`** цикъла, вложени един в друг, по един за всеки символ. За да гарантираме условието последния символ, който по условие е число, да бъде **по-голям** от първите два, които също са числа, ще използваме вградения метод **`Math.max(…)`**: 
...
Решете задачата самостоятелно използвайки наученото от предишните две. Спомнете си разликата между операторите за **целочислено деление ( __/__ )** и **деление с остатък ( __%__ )** в JavaScript.
