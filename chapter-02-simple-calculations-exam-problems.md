...
Необходима ни е функция, която приема един аргумент `arg1`. В нея ще създадем променлива, в която да запазим числото (напр. **`num`**), и да използваме стандартната команда за четене на данни от конзолата в съчетание с функцията **`parseInt(…)`**, която конвертира текст в число:
