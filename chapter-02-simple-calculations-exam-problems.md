...
В условието на задачата е зададено закръгляне на изхода до втория знак след десетичната запетая. Затова не можем просто отпечатаме стойностите с **`console.log(…)`**. Можем да използваме функцията **`Math.round(…)`**, която закръгля подаденото число до най-близкото цяло число. За да не загубим двата знака след запетаята, ще приложим функцията върху числото умножено по 100, а после ще разделим получения резултат на 100.
