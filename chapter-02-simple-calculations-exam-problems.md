...
**Placeholder** представлява израз, който ще бъде заменен с конкретна стойност при отпечатване. За да работи шаблонът, трябва да използваме наклонени кавички (апострофи) **`` `…` ``**. Методът **`console.log(…)`** поддържа печатане на текст по шаблон, като аргументите, които трябва да отпечатаме, се задават в **`${…}`**:
