...
Необходима ни е функция, на която се подава един аргумент **`arg1`**. В нея ще създадем променлива, в която да запазим числото (напр. **`num`**), в съчетание с функцията **`parseInt(…)`**, която конвертира текст в число:
...
**Placeholder** представлява израз, който ще бъде заменен с конкретна стойност при отпечатване. За да работи шаблонът, трябва да използваме наклонени кавички (апострофи) **`` `…` ``**. Функцията **`console.log(…)`** поддържа печатане на текст по шаблон, като аргументите, които трябва да отпечатаме, се задават в **`${…}`**:
...
Първото, което трябва да направим, е да вземем входните данни. Създаваме си функция, на която се подават два аргумента. Запазваме техните стойности в две променливи, като използваме функцията **`parseFloat(…)`** за преобразуване на зададената стрингова (текстова) стойност в дробно число. 
...
В случая може да пробваме със следното търсене: "[***JavaScript get whole number part of double***](https://www.google.com/?q=javascript+get+whole+number+part+of+double)". Откриваме, че една от възможностите е да използваме функцията **`Math.trunc(…)`**. Кодът по-долу е целенасочено замъглен и трябва да бъде довършен от читателя:
...
За да прочетем входните данни, декларираме променливи, като внимаваме да ги именуваме по такъв начин, който да ни подсказва какви стойности съдържат променливите. С функциите **`parseInt(…)`** и **`parseFloat(…)`** преобразуваме зададената текстова стойност съответно в цяло и дробно число.
...
Дължината на страна от площадката, широчината и дължината на пейката ще бъдат дадени като **цели числа**, следователно, за да запазим техните стойности може да декларираме **променливи преобразувани с функцията `parseInt(…)`**. За широчината и дължината на плочките ще ни бъдат подадени реални числа (с цяла и дробна част), затова за тях ще използваме **`parseFloat(…)`**. 
...
В условието на задачата е зададено закръгляне на изхода **до втория знак след десетичната запетая**. Затова не можем просто отпечатаме стойностите с **`console.log(…)`**. Ще използваме функцията **`Math.round(…)`**, която закръгля подаденото число до най-близкото цяло число. За да не загубим двата знака след запетаята, прилагаме функцията върху числото умножено по 100, а после делим получения резултат на 100:
...
|------------|------------------|--------------|------------------|
|20<br>5678<br>2.4|12442.2442010256|7<br>50200.12<br>3|10659.47|
...
**Биткойните** са дадени като **цяло число**, следователно за тяхната стойност може да декларираме променлива преобразувана с функцията **`parseInt(…)`**. Като брой **китайски юани и комисионна** ще получим **реално число**, следователно за тях използваме **`parseFloat(…)`**.
...
**Работните дни за месец** са дадени като **цяло число**, следователно за тяхната стойност може да декларираме променлива, в която да конвертираме до число с функцията **`parseInt(…)`**. За **изкараните пари**, както и за **курса на долара спрямо лева**, ще получим **реално число**, следователно за тях използваме **`parseFloat(…)`**. 
...
Накрая остава да изведем резултата на екрана. Забелязваме, че се **изисква форматиране на числената стойност до втория знак след десетичната точка**. Можем да използваме функцията **`.toFixed(…)`** по същия начин като в предходната задача:
