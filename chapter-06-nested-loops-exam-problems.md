...
От условието на задачата виждаме, че **входните данни** ще се състоят само от едно **цяло число** в интервала [**3 … 1000**]. Създаваме функция, която приема като аргумент **масив от един елемент**. Тъй като той е от тип **стринг**, а ние трябва да работим с числа, използваме функцията `Number()`, с която да го конвертираме: 
