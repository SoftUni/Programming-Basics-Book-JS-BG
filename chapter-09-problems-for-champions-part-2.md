...
*Може да замените този __`while`__ с __`for`__ цикък използвайки само условието и стъпката на __`for`__*.
...
* Можем да използваме функцията **`str.toUpperCase()`** и да сравним дали символът е същия, като този, който ще получим от **`str.toUpperCase()`**
...
*Защото при деление получаваме дробно число или трябва да премахнем дробната част преди да разделим модулно на 10 чрез __`Math.floor(...)`__ или чрез кастване към цяло число чрез __`parseInt(...)`__*
